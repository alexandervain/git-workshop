...
export interface ExcerciseContext {
  id: string;
  folderName: string;
  folderPath: string;
...
interface ExcerciseConfig {
  context: ExcerciseContext;
  topic: string;
  tldr: string;
  description: string;
  tasks: ExcerciseTask[];
...
export class Excercise {
  ...
  public constructor(private readonly config: ExcerciseConfig) {
      ...
      .action(async () => {
        ...
        if (!folderExists) {
          results.push({
            passed: false,
            text: `state`,
            failureDetails: `folder ${this.folderPath} not found - make sure you run and completed excersise #${this.id} tasks`,
          });
        } else {
  ...
  private async startEx() {
    if (await exists(this.folderPath)) {
      console.log(
        chalk.redBright(
          `\n>>> The excercise #${this.id} is already in progress <<<\n`
        )
      );
      ...
      console.log(
        ` - ${chalk.italic(
          chalk.green("print")
        )} command to just print the excercise's details again`
      );
      console.log();
      return;
    }
